...
router.get('/', verifyGuest, async (req, res) => {
    try {
        ...
        return res.status(200).json({ success: true, message: 'order_listed', data: "req" });
...
router.get('/list', verifyGuest, async (req, res) => {
    try {
        ...
        var user = await Guest.findOne({ session_token:req.session_token });
        if (user) {
...
router.get('/details', verifyGuest, async (req, res) => {
    try {
...
router.post('/create', verifyGuest, async (req, res) => {
    try {
        ...
        let { address, coupon_code, card, delivery_type, email, mobile} = req.body;
        ...
        var ex_user = await User.findOne({ email:email });
        if(ex_user){
            return res.status(200).json({ success: false, message: await Language.getResponse(req.session.language, 'kindly_login_your_account_to_place_order') });
        }

        let guest_limit = await Order.find({email:email});
        if(guest_limit.length >= 3){
            return res.status(200).json({ success: false, message: await Language.getResponse(req.session.language, 'guest_limit_over') });
        }
        ...
        if (user) {
            ...
            if (card) {
                let addMoney = await Stripe.getCardToken(card.number, card.month, card.year,card.cvc,user.email,amount_paid,'Payment for Order id : ' + tx_id + ' !', 'eur');
...
router.get('/paynow', verifyGuest, async (req, res) => {
    try {

        var data = await Stripe.getCardToken('4242424242424242', '02', "2021", "123","demo@demo.com",100);

        return res.status(200).json({ success: true, message: await Language.getResponse(req.session.language, 'order_listed'),data:data });
